...
<script>
    jQuery(document).ready(function() {
        ...
        jQuery(document).on('click', '#delete_data', function(e) {
            ...
            jQuery.ajax({
                ...
                success: function(response)
                {
                    ...
                    {
                        jQuery('#deleteMsg').html('<lable class="text-success">'+response.msg+'</label>');
                        jQuery('.delete-'+id).hide();
                        jQuery('#delete_data').hide();
                    }
                    else
                    {
                        jQuery('#deleteMsg').html('<lable class="text-danger">'+response.msg+'</label>');
                    }
                },
                complete:function(data)
                {
        ...
        /* function search_data()
        {
            ...
            var urlUri = 'searchKeryword='+searchKeryword+'&role='+role+'&status='+status;
            jQuery.ajax({
                url: "<?php //echo route('admin.users.index'); ?>?"+urlUri,
                cache: false,
                beforeSend: function(){
                    // Show image container
                    jQuery("#loader").show();
    ...
    /* function changeUserStatus( _this, id )
    {
        ...
        jQuery.ajax({
            url: "<?php //echo route('admin.users.status'); ?>",
            type: 'post',
            cache: false,
            data: {
                _token: _token,
            ...
            success: function(response)
            {
                if( response.msgType == 'success' )
                {
                    jQuery('.status-msg-'+id).html('<lable class="text-success">'+response.msg+'</label>');
                }
                else
                {
                    jQuery('.status-msg-'+id).html('<lable class="text-danger">'+response.msg+'</label>');
                }
